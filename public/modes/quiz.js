...
// أداة مساعدة لتوحيد وتنسيق مصفوفة الإجابات الصحيحة (تحويل لأرقام وترتيب)
const normalizeAnswers = (arr) => arr.map(Number).filter(n => !isNaN(n)).sort((a, b) => a - b);
...
export function updateQuestionNumbers(container) {
    ...
    blocks.forEach((blockEl, index) => {
        const span = blockEl.querySelector('.q-number');
    ...
    save() {
        ...
        const draftData = {
            ...
            questions: Array.from(blocks).map(block => {
                ...
                return {
                    q: block.querySelector('.mcq-q-text').value.trim(),
                    opts: [
                        block.querySelector('.mcq-opt-0').value.trim(),
                        block.querySelector('.mcq-opt-1').value.trim(),
                        block.querySelector('.mcq-opt-2').value.trim(),
                        block.querySelector('.mcq-opt-3').value.trim()
                    ],
                    correctAnswers: normalizeAnswers(Array.from(correctCheckboxes).map(cb => cb.value))
                };
            })
        };
                    ...
                    if (yes) {
                        ...
                        data.questions.forEach((qData, idx) => {
                            setTimeout(() => {
                                ...
                                [0, 1, 2, 3].forEach(i => {
                                    lastBlock.querySelector(`.mcq-opt-${i}`).value = qData.opts[i] || '';
                                });
                                
                                const correctArr = qData.correctAnswers || (qData.correct !== undefined ? [parseInt(qData.correct)] : []);
                                normalizeAnswers(correctArr).forEach(idx => {
                                    const chk = lastBlock.querySelector(`.mcq-correct-chk[value="${idx}"]`);
                                    if (chk) chk.checked = true;
                                });
                            }, idx * 100); // تأثير استعادة متدرج
                        });
                        setTimeout(() => SysUI.toast('success', 'تم استعادة المسودة بنجاح! 🚀'), data.questions.length * 100 + 100);
                    } else {
...
function generateBlockHTML(counter, isPublic = false) {
    const config = isPublic 
        ? { title: 'السؤال العام', color: 'blue', textGradient: 'from-blue-400 to-indigo-500', shadow: 'rgba(59,130,246,0.3)', placeholder: 'اكتب نص السؤال العام هنا (أو الصق بذكاء)...' }
        : { title: 'السؤال', color: 'yellow', textGradient: 'from-yellow-400 to-yellow-600', shadow: 'rgba(234,179,8,0.3)', placeholder: 'اكتب نص السؤال الأساسي هنا (أو الصق السؤال بالكامل ليتم توزيعه تلقائياً)...' };
    ...
    return `
        <div class="flex justify-between items-center mb-5 pb-3 border-b border-white/10 group-hover:border-white/20 transition-colors">
            <h3 class="text-base sm:text-lg font-bold text-transparent bg-clip-text bg-gradient-to-r ${config.textGradient} flex items-center gap-3">
                ...
                ${config.title} رقم <span class="q-number bg-${config.color}-500/20 text-${config.color}-400 px-2.5 py-0.5 rounded-md">${counter}</span>
            </h3>
            <button onclick="removeBlock(this.closest('.${isPublic ? 'public-' : ''}mcq-block'))" class="trash-icon text-gray-500 hover:text-white hover:bg-red-500/80 transition-all duration-300 cursor-pointer p-2.5 rounded-xl shadow-sm hover:shadow-[0_0_15px_rgba(239,68,68,0.5)] flex items-center justify-center" title="حذف السؤال">
                ${trashSVG}
            </button>
        </div>
        
        <div class="relative mb-5 group/textarea">
            <div class="absolute -inset-0.5 bg-gradient-to-r from-${config.color}-500/20 to-purple-500/20 rounded-2xl blur opacity-0 group-focus-within/textarea:opacity-100 transition duration-500"></div>
            <textarea class="mcq-q-text relative w-full bg-black/60 border border-white/10 rounded-2xl px-5 py-4 text-white outline-none focus:border-${config.color}-500/50 transition-all text-sm sm:text-base resize-none overflow-hidden placeholder-gray-500 leading-relaxed custom-scrollbar shadow-inner" rows="2" placeholder="${config.placeholder}" required></textarea>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            ${[0, 1, 2, 3].map(i => `
                <div class="relative flex items-center group/opt">
                    ...
                    <span class="absolute right-4 text-gray-500 font-black text-sm group-focus-within/opt:text-${config.color}-400 transition-colors pointer-events-none">${['أ', 'ب', 'ج', 'د'][i]}</span>
                    <input type="text" class="mcq-opt-${i} relative w-full bg-black/40 border border-white/5 rounded-xl pr-10 pl-4 py-3.5 text-white outline-none focus:border-${config.color}-500/50 focus:bg-black/60 transition-all text-sm placeholder-gray-600 shadow-inner" placeholder="الخيار ${['الأول', 'الثاني', 'الثالث', 'الرابع'][i]}" ${i < 2 ? 'required' : ''}>
                </div>
            `).join('')}
        </div>
        
        <div class="bg-gradient-to-r ${isPublic ? 'from-indigo-900/30' : 'from-emerald-900/30'} to-black/30 border ${isPublic ? 'border-indigo-500/20' : 'border-emerald-500/20'} p-4 sm:p-5 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-inner">
            <div class="flex flex-col gap-1">
                <label class="text-sm font-bold ${isPublic ? 'text-indigo-400' : 'text-emerald-400'} flex items-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                    الإجابات الصحيحة ${isPublic ? '(للعام)' : ''}
                </label>
                <span class="text-xs ${isPublic ? 'text-indigo-500/70' : 'text-emerald-500/70'}">يمكنك تحديد أكثر من إجابة (خيارين أو 4)</span>
            </div>
                ...
                ${[0, 1, 2, 3].map(i => `
                    <label class="cursor-pointer flex-1 sm:flex-none relative group/chk">
                        ...
                        <div class="flex items-center justify-center min-w-[3rem] px-4 py-2.5 rounded-xl bg-black/40 border border-white/10 text-gray-400 font-bold text-sm transition-all duration-300 
                                    peer-checked:${isPublic ? 'bg-indigo-500/20 border-indigo-500 text-indigo-400 shadow-[0_0_15px_rgba(99,102,241,0.3)]' : 'bg-emerald-500/20 border-emerald-500 text-emerald-400 shadow-[0_0_15px_rgba(16,185,129,0.3)]'}
                                    hover:${isPublic ? 'border-indigo-500/50' : 'border-emerald-500/50'} hover:bg-white/5">
                            ${['أ', 'ب', 'ج', 'د'][i]}
                        </div>
                    </label>
                `).join('')}
...
function handlePasteExtraction(e, blockTypeClass, highlightColorClass, shadowColor) {
    let pasteText = (e.clipboardData || window.clipboardData).getData('text');
    let lines = pasteText.split('\n').map(line => line.trim()).filter(line => line.length > 0);
    
    // دعم ذكي لللصق السريع (إما سؤال بخيارين أو 4 خيارات)
    if (lines.length === 3 || lines.length >= 5) {
        e.preventDefault(); 
        e.target.value = lines[0]; 
        autoResizeTextarea(e.target);
        
        const currentBlock = e.target.closest(`.${blockTypeClass}`);
        currentBlock.querySelector('.mcq-opt-0').value = lines[1].replace(/^[أ-د][.-]\s*/, '').trim(); 
        currentBlock.querySelector('.mcq-opt-1').value = lines[2].replace(/^[أ-د][.-]\s*/, '').trim();
        
        if (lines.length >= 5) {
            currentBlock.querySelector('.mcq-opt-2').value = lines[3].replace(/^[أ-د][.-]\s*/, '').trim();
            currentBlock.querySelector('.mcq-opt-3').value = lines[4].replace(/^[أ-د][.-]\s*/, '').trim();
        } else {
            currentBlock.querySelector('.mcq-opt-2').value = '';
            currentBlock.querySelector('.mcq-opt-3').value = '';
        }
        
        currentBlock.classList.add('ring-2', highlightColorClass, shadowColor, 'scale-[1.01]');
        setTimeout(() => currentBlock.classList.remove('ring-2', highlightColorClass, shadowColor, 'scale-[1.01]'), 1200);
        SysUI.toast('success', '✨ تم استخراج السؤال والخيارات بذكاء!');
        SysUI.confetti();
    }
}

function attachDragAndDrop(block, container, blockTypeClass, highlightBorder, highlightBg) {
    if (window.innerWidth <= 768) return;

    block.addEventListener('dragstart', function(e) {
        this.classList.add('opacity-40', 'border-dashed', highlightBorder, 'scale-95');
    ...
    block.addEventListener('dragover', function(e) {
        ...
        if (window.draggedBlock !== this) {
            this.classList.add(highlightBorder, highlightBg, '-translate-y-2');
        }
    });
    block.addEventListener('dragleave', function(e) {
        this.classList.remove(highlightBorder, highlightBg, '-translate-y-2');
    });
    block.addEventListener('drop', function(e) {
        e.preventDefault();
        this.classList.remove(highlightBorder, highlightBg, '-translate-y-2');
        if (window.draggedBlock && window.draggedBlock !== this) {
            const allBlocks = [...container.querySelectorAll(`.${blockTypeClass}`)];
    ...
    block.addEventListener('dragend', function(e) {
        this.classList.remove('opacity-40', 'border-dashed', highlightBorder, 'scale-95');
...
export function addMCQBlock() {
    ...
    block.innerHTML = generateBlockHTML(questionCounter, false);
    ...
    questionTextarea.addEventListener('paste', (e) => handlePasteExtraction(e, 'mcq-block', 'ring-emerald-500', 'shadow-[0_0_30px_rgba(16,185,129,0.3)]'));

    attachDragAndDrop(block, container, 'mcq-block', 'border-yellow-500', 'bg-yellow-500/5');
...
export function addPublicMCQBlock() {
    ...
    block.style.opacity = "0";
    block.style.transform = "translateY(20px) scale(0.98)";
    block.innerHTML = generateBlockHTML(publicQuestionCounter, true);

    const questionTextarea = block.querySelector('.mcq-q-text');
    questionTextarea.addEventListener('input', () => autoResizeTextarea(questionTextarea));
    questionTextarea.addEventListener('paste', (e) => handlePasteExtraction(e, 'public-mcq-block', 'ring-blue-500', 'shadow-[0_0_30px_rgba(59,130,246,0.3)]'));

    attachDragAndDrop(block, container, 'public-mcq-block', 'border-blue-500', 'bg-blue-500/5');
...
const validateAndExtractQuestions = (blocks, isPublic = false) => {
    const questions = [];
    let hasError = false;
    ...
    blocks.forEach((block, index) => {
        ...
        const qText = block.querySelector('.mcq-q-text').value.trim();
        ...
        const correctCheckboxes = block.querySelectorAll('.mcq-correct-chk:checked');
        let correctAnswers = normalizeAnswers(Array.from(correctCheckboxes).map(cb => cb.value));
    ...
    return hasError ? null : questions;
};
...
if (quizForm) {
    quizForm.addEventListener('submit', async (e) => {
        ...
        if (!quizTitle) return SysUI.toast('warning', "⚠️ يرجى كتابة عنوان الاختبار أولاً!");
        if (!quizGrade) return SysUI.toast('warning', "⚠️ يرجى اختيار الصف الدراسي!");

        const blocks = document.querySelectorAll('#dynamicQuestionsContainer .mcq-block');
        if (blocks.length === 0) return SysUI.toast('warning', "⚠️ أضف سؤالاً واحداً على الأقل!"); 
        
        const questions = validateAndExtractQuestions(blocks, false);
        if (!questions) return;
...
if (publicQuizForm) {
    publicQuizForm.addEventListener('submit', async (e) => {
        ...
        if (!publicQuizTitle) return SysUI.toast('warning', "⚠️ يرجى كتابة عنوان الاختبار العام أولاً!");

        const blocks = document.querySelectorAll('#dynamicPublicQuestionsContainer .public-mcq-block');
        if (blocks.length === 0) return SysUI.toast('warning', "⚠️ أضف سؤالاً واحداً على الأقل!"); 
        
        const questions = validateAndExtractQuestions(blocks, true);
        if (!questions) return;
...
export const SmartImportSystem = {
    ...
    processImport() {
        ...
        if (!rawText) return SysUI.toast('error', '⚠️ الحقل فارغ تماماً.');

        const parsedQuestions = this.parseText(rawText);
        
        if (parsedQuestions.length === 0) {
            return SysUI.toast('error', '❌ لم أتمكن من التعرف على أي أسئلة صحيحة (خيارين أو 4 فقط). تأكد من التنسيق.');
        }
    ...
    parseText(text) {
        ...
        const finalizeQuestion = () => {
            if (currentQ.opts.length >= 2) {
                currentQ.q = currentQ.q.replace(/^(س\d+[:.-]?|السؤال \d+[:.-]?)\s*/, '').trim();
                // حماية وتأكيد: خيارين أو 4 فقط دون خيارات وهمية
                if (currentQ.opts.length > 4) currentQ.opts = currentQ.opts.slice(0, 4);
                
                if (currentQ.opts.length === 2 || currentQ.opts.length === 4) {
                    currentQ.correctAnswers = normalizeAnswers(currentQ.correctAnswers);
                    parsed.push({ ...currentQ });
                }
            }
            currentQ = { q: '', opts: [], correctAnswers: [] };
        };
        ...
        lines.forEach(line => {
            ...
            if (match) {
                ...
                if (isCorrect) currentQ.correctAnswers.push(currentQ.opts.length - 1);
            } else {
    ...
    async animateInsertion(questions) {
        ...
        for (let i = 0; i < questions.length; i++) {
            ...
            requestAnimationFrame(() => {
                scanner.style.transition = 'width 1s cubic-bezier(0.25, 1, 0.5, 1)';
            ...
            const optInputs = [0, 1, 2, 3].map(idx => targetBlock.querySelector(`.mcq-opt-${idx}`));
